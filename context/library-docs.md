...
- `sanitize-html` (apps/web only, `lib/sanitize.ts` `sanitizeBlogHtml`): blog bodies are admin-authored rich-text HTML but are rendered into a public page, so they are sanitized at the point of output with a tag/attribute allow-list (no scripts, handlers, iframes, `data:` images or `javascript:` links; `<h1>` demoted to `<h2>`; external links get `target="_blank" rel="noopener noreferrer"`). Pure JS, no DOM needed, so it runs in a Server Component. Never render `Blog.content` (or any other admin HTML) with `dangerouslySetInnerHTML` without going through it.
- Fonts (both apps): **self-hosted with `next/font/local`, never `next/font/google`.** Inter and JetBrains Mono are the variable latin-subset woff2 files in `apps/{web,admin}/app/fonts/` (from `@fontsource-variable/*`, SIL OFL 1.1 — the licence text sits beside them and must stay). Reason: the Google loader downloads at compile time, and under Turbopack a slow/timed-out download is a hard "Can't resolve '@vercel/turbopack-next/internal/font/google/font'" build error that 500s every page (it took the admin down on 2026-09-19). The `--font-sans` / `--font-mono` CSS variables and weights are unchanged; to add a font, drop its woff2 (and licence) into `app/fonts/` and wrap it with `localFont`.
