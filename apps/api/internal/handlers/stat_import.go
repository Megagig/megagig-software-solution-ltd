...
// Import kicks off a BACKGROUND CSV import of stats. It streams the upload
...
func (h *StatHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsStat(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "stats", Status: "processing", Total: total}
	...
	go h.runImportStat(job.ID, tmpPath)
...
// countCSVRowsStat counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsStat(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportStat streams the temp CSV, creating stats in batches and
...
func (h *StatHandler) runImportStat(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Stat, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing stats.
...
func (h *StatHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="stats-template.csv"`)
	c.String(http.StatusOK, "value,label,published,sort_order\n")
}
