...
// Import kicks off a BACKGROUND CSV import of case_studies. It streams the upload
...
func (h *CaseStudyHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsCaseStudy(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "case_studies", Status: "processing", Total: total}
	...
	go h.runImportCaseStudy(job.ID, tmpPath)
...
// countCSVRowsCaseStudy counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsCaseStudy(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportCaseStudy streams the temp CSV, creating case_studies in batches and
...
func (h *CaseStudyHandler) runImportCaseStudy(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.CaseStudy, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.CaseStudy{}
		if v, ok := get(rec, "slug"); ok {
			item.Slug = v
		}
		if v, ok := get(rec, "client_name"); ok {
			item.ClientName = v
		}
		if v, ok := get(rec, "tagline"); ok {
			item.Tagline = v
		}
		if v, ok := get(rec, "status_badge"); ok {
			item.StatusBadge = v
		}
		if v, ok := get(rec, "hero_image"); ok && v != "" {
			var rel models.Upload
			if err := h.DB.Where("filename = ?", v).First(&rel).Error; err != nil {
				rel = models.Upload{Filename: v}
				h.DB.Create(&rel)
			}
			item.HeroImageID = rel.ID
		}
		if v, ok := get(rec, "problem"); ok {
			item.Problem = v
		}
		if v, ok := get(rec, "what_we_built"); ok {
			item.WhatWeBuilt = v
		}
		if v, ok := get(rec, "result"); ok {
			item.Result = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing case_studies.
...
func (h *CaseStudyHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="case-studies-template.csv"`)
	c.String(http.StatusOK, "slug,client_name,tagline,status_badge,hero_image,problem,what_we_built,result,published,sort_order\n")
}
