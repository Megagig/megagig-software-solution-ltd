...
// Import kicks off a BACKGROUND CSV import of testimonials. It streams the upload
...
func (h *TestimonialHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsTestimonial(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "testimonials", Status: "processing", Total: total}
	...
	go h.runImportTestimonial(job.ID, tmpPath)
...
// countCSVRowsTestimonial counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsTestimonial(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportTestimonial streams the temp CSV, creating testimonials in batches and
...
func (h *TestimonialHandler) runImportTestimonial(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Testimonial, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.Testimonial{}
		if v, ok := get(rec, "quote_text"); ok {
			item.QuoteText = v
		}
		if v, ok := get(rec, "author_name"); ok {
			item.AuthorName = v
		}
		if v, ok := get(rec, "author_role"); ok {
			item.AuthorRole = v
		}
		if v, ok := get(rec, "company_name"); ok {
			item.CompanyName = v
		}
		if v, ok := get(rec, "company_url"); ok {
			item.CompanyURL = v
		}
		if v, ok := get(rec, "avatar"); ok && v != "" {
			var rel models.Upload
			if err := h.DB.Where("filename = ?", v).First(&rel).Error; err != nil {
				rel = models.Upload{Filename: v}
				h.DB.Create(&rel)
			}
			item.AvatarID = rel.ID
		}
		if v, ok := get(rec, "case_study"); ok && v != "" {
			var rel models.CaseStudy
			if err := h.DB.Where("slug = ?", v).First(&rel).Error; err != nil {
				rel = models.CaseStudy{Slug: v}
				h.DB.Create(&rel)
			}
			item.CaseStudyID = rel.ID
		}
		if v, ok := get(rec, "published"); ok {
			item.Published = v == "true" || v == "1" || v == "yes"
		}
		if v, ok := get(rec, "sort_order"); ok {
			n, _ := strconv.Atoi(v)
			item.SortOrder = n
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing testimonials.
...
func (h *TestimonialHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="testimonials-template.csv"`)
	c.String(http.StatusOK, "quote_text,author_name,author_role,company_name,company_url,avatar,case_study,published,sort_order\n")
}
