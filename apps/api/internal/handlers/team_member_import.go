...
// Import kicks off a BACKGROUND CSV import of team_members. It streams the upload
...
func (h *TeamMemberHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsTeamMember(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "team_members", Status: "processing", Total: total}
	...
	go h.runImportTeamMember(job.ID, tmpPath)
...
// countCSVRowsTeamMember counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsTeamMember(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportTeamMember streams the temp CSV, creating team_members in batches and
...
func (h *TeamMemberHandler) runImportTeamMember(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.TeamMember, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing team_members.
