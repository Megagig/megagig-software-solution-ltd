...
// Import kicks off a BACKGROUND CSV import of about_items. It streams the upload
...
func (h *AboutItemHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsAboutItem(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "about_items", Status: "processing", Total: total}
	...
	go h.runImportAboutItem(job.ID, tmpPath)
...
// countCSVRowsAboutItem counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsAboutItem(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportAboutItem streams the temp CSV, creating about_items in batches and
...
func (h *AboutItemHandler) runImportAboutItem(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.AboutItem, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing about_items.
...
func (h *AboutItemHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="about-items-template.csv"`)
	c.String(http.StatusOK, "kind,title,description,label,published,sort_order\n")
}
