...
// Import kicks off a BACKGROUND CSV import of faqs. It streams the upload
...
func (h *FAQHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsFAQ(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "faqs", Status: "processing", Total: total}
	...
	go h.runImportFAQ(job.ID, tmpPath)
...
// countCSVRowsFAQ counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsFAQ(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportFAQ streams the temp CSV, creating faqs in batches and
...
func (h *FAQHandler) runImportFAQ(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.FAQ, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.FAQ{}
		if v, ok := get(rec, "question"); ok {
			item.Question = v
		}
		if v, ok := get(rec, "answer"); ok {
			item.Answer = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing faqs.
...
func (h *FAQHandler) Template(c *gin.Context) {
	...
	c.Header("Content-Disposition", `attachment; filename="faqs-template.csv"`)
	c.String(http.StatusOK, "question,answer,published,sort_order\n")
}
